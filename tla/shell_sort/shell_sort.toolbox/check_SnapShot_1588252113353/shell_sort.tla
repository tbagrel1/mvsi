...
Pl12 ==
    ...
    /\ Enum(n, tab_out) = FuncPM(Enum(n, tab), Ind({ tab_out[j] }), Ind({ tmp }))
    /\ (inc = 1 => (Forall({ tab_out[k] \leq tab_out[k + 1] : k \in 0..i - 2 }) /\ (j = i \/ tab_out[i - 1] \leq tab_out[i])))
Pl13 ==
    /\ 0..n - 1 \subseteq DOMAIN tab_out /\ inc > 0 /\ 0 \leq i /\ i < n
    /\ inc \leq j /\ j \leq i /\ tab_out[j - inc] \geq tmp
    /\ Enum(n, tab_out) = FuncPM(Enum(n, tab), Ind({ tab_out[j - inc] }), Ind({ tmp }))
...
\* Last modified Thu Apr 30 15:08:16 CEST 2020 by thomas
