...
Ind(S) == [m \in Range(tab) |-> IF m \in S THEN 1 ELSE 0 ]
...
Pl7 ==
    ...
    /\ Forall({ Forall({tab_out[l] \leq tab_out[k] : l \in 0..k - 1 }) : k \in n - c..n - 1})
Pl8 ==
    /\ 0..n - 1 \subseteq DOMAIN tab_out /\ 0 \leq c /\ c < n - 1 /\ Enum(n, tab_out) = Enum(n, tab)
    /\ Forall({ Forall({tab_out[l] \leq tab_out[k] : l \in 0..k - 1 }) : k \in n - c..n - 1}) /\ d = 0
Pl9 ==
    /\ 0..n - 1 \subseteq DOMAIN tab_out /\ 0 \leq c /\ c < n - 1 /\ Enum(n, tab_out) = Enum(n, tab)
    /\ Forall({ Forall({tab_out[l] \leq tab_out[k] : l \in 0..k - 1 }) : k \in n - c..n - 1}) 
    /\ 0 \leq d /\ d < n - c - 1 /\ Forall({ tab_out[k] \leq tab_out[d] : k \in 0..d - 1})
Pl10 ==
    /\ 0..n - 1 \subseteq DOMAIN tab_out /\ 0 \leq c /\ c < n - 1 /\ Enum(n, tab_out) = Enum(n, tab)
    /\ Forall({ Forall({tab_out[l] \leq tab_out[k] : l \in 0..k - 1 }) : k \in n - c..n - 1}) 
    /\ 0 \leq d /\ d < n - c - 1 /\ Forall({ tab_out[k] \leq tab_out[d] : k \in 0..d - 1})
...
Pl11 ==
    /\ 0..n - 1 \subseteq DOMAIN tab_out /\ 0 \leq c /\ c < n - 1 /\ Enum(n, tab_out) = Enum(n, tab)
    /\ Forall({ Forall({tab_out[l] \leq tab_out[k] : l \in 0..k - 1 }) /\ swap \leq tab_out[k] : k \in n - c..n - 1 }) 
    /\ 0 \leq d /\ d < n - c - 1 /\ Forall({ tab_out[k] \leq tab_out[d] : k \in 0..d - 1 })
    /\ tab_out[d] > tab_out[d + 1] /\ swap = tab_out[d] /\ swap > tab_out[d + 1]
Pl12 ==
    /\ 0..n - 1 \subseteq DOMAIN tab_out /\ 0 \leq c /\ c < n - 1 
    /\ Enum(n, tab_out) = FuncPM(Enum(n, tab), Ind({ tab_out[d + 1] }), Ind({ swap }))
    /\ Forall({ Forall({tab_out[l] \leq tab_out[k] : l \in 0..k }) /\ swap \leq tab_out[k] : k \in n - c..n - 1 }) 
    /\ 0 \leq d /\ d < n - c - 1/\ Forall({ tab_out[k] \leq swap : k \in 0..d + 1})
Pl13 ==
    /\ 0..n - 1 \subseteq DOMAIN tab_out /\ 0 \leq c /\ c < n - 1 /\ Enum(n, tab_out) = Enum(n, tab)
    /\ Forall({ Forall({tab_out[l] \leq tab_out[k] : l \in 0..k - 1 }) : k \in n - c..n - 1}) 
    /\ 0 \leq d /\ d < n - c - 1/\ Forall({ tab_out[k] \leq tab_out[d + 1] : k \in 0..d})
Pl14 ==
    /\ 0..n - 1 \subseteq DOMAIN tab_out /\ 0 \leq c /\ c < n - 1 /\ Enum(n, tab_out) = Enum(n, tab)
    /\ Forall({ Forall({tab_out[l] \leq tab_out[k] : l \in 0..k - 1 }) : k \in n - c..n - 1}) 
    /\ 0 \leq d /\ d < n - c - 1/\ Forall({ tab_out[k] \leq tab_out[d + 1] : k \in 0..d})
...
Pl16 ==
    /\ 0..n - 1 \subseteq DOMAIN tab_out /\ 0 \leq c /\ c < n - 1 /\ Enum(n, tab_out) = Enum(n, tab)
    /\ Forall({ Forall({tab_out[l] \leq tab_out[k] : l \in 0..k - 1 }) : k \in n - c..n - 1}) 
    /\ 0 \leq d /\ d \leq n - c - 1/\ Forall({ tab_out[k] \leq tab_out[d] : k \in 0..d - 1})
Pl17 ==
    /\ 0..n - 1 \subseteq DOMAIN tab_out /\ 0 \leq c /\ c < n - 1 /\ Enum(n, tab_out) = Enum(n, tab)
    /\ Forall({ Forall({tab_out[l] \leq tab_out[k] : l \in 0..k - 1 }) : k \in n - c - 1..n - 1}) 
Pl18 ==
...
\* Last modified Thu Apr 30 21:46:24 CEST 2020 by thomas
\* Last modified Thu Apr 30 14:46:54 CEST 2020 by alexandre
